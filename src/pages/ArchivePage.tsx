...
import { getSeasonSummaries, hasFullDetail } from '@/data/catalog'
    ...
    <div style={{ display: 'grid', gap: '1rem' }}>
      <header>
        ...
        <p className="muted" style={{ margin: 0 }}>
          2016 两场 · 2017 三场 · 其余年份四场。历史赛季为摘要数据；2026 夏季赛提供完整明细。
        </p>
      </header>

      <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
        {years.map((y) => (
          <button key={y} type="button" className={`tab${year === y ? ' active' : ''}`} onClick={() => setYear(y)}>
            {y}
          ...
          <article key={s.id} className="card card-pad" style={{ display: 'grid', gap: '0.65rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
              <div>
                ...
                <h2 style={{ margin: 0, fontSize: '1.2rem' }}>{s.name}</h2>
              </div>
